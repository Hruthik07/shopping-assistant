...
async def test_aggregator_search_products():
    ...
    results = await product_aggregator.search_products(query=query, num_results=5)
...
async def test_aggregator_price_filtering():
    ...
    results = await product_aggregator.search_products(
        query=query, num_results=10, min_price=500.0, max_price=1000.0
    )
...
async def test_aggregator_category_filtering():
    ...
    query = "headphones"
    results = await product_aggregator.search_products(
        query=query, num_results=5, category="electronics"
    )
...
async def test_aggregator_deduplication():
    ...
    query = "iPhone"
    results = await product_aggregator.search_products(query=query, num_results=10)
...
async def test_aggregator_empty_query():
    ...
    results = await product_aggregator.search_products(query="", num_results=5)
