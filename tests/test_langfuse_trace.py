    ...
    async def test_trace_creation_success(self, agent):
        ...
        with patch.object(langfuse_client, 'enabled', True):
            with patch.object(langfuse_client, 'trace') as mock_trace:
                ...
                with patch.object(agent, '_get_conversation_history', new_callable=AsyncMock) as mock_history:
                    with patch.object(agent, '_get_user_preferences', new_callable=AsyncMock) as mock_prefs:
                        with patch.object(agent, '_detect_intent') as mock_intent:
                            with patch.object(agent, '_build_messages') as mock_messages:
                                with patch.object(agent, '_create_context_hash') as mock_hash:
                                    with patch.object(agent, '_process_with_llm', new_callable=AsyncMock) as mock_llm:
                                        with patch.object(agent, '_store_conversation', new_callable=AsyncMock) as mock_store:
                                            with patch.object(agent, '_track_analytics') as mock_analytics:
                                                ...
                                                mock_llm.return_value = ("Test response", [], ["search_products"], None)
                                                ...
                                                assert result.get("langfuse_trace_id") == "test-trace-id-123"
    
    @pytest.mark.asyncio
    async def test_trace_creation_failure_graceful(self, agent):
        """Test that trace creation failure doesn't break the query."""
        with patch.object(langfuse_client, 'enabled', True):
            with patch.object(langfuse_client, 'trace', side_effect=Exception("Trace creation failed")):
                    ...
                    with patch.object(agent, '_get_user_preferences', new_callable=AsyncMock) as mock_prefs:
                        with patch.object(agent, '_detect_intent') as mock_intent:
                            with patch.object(agent, '_build_messages') as mock_messages:
                                with patch.object(agent, '_create_context_hash') as mock_hash:
                                    with patch.object(agent, '_process_with_llm', new_callable=AsyncMock) as mock_llm:
                                        with patch.object(agent, '_store_conversation', new_callable=AsyncMock):
                                            with patch.object(agent, '_track_analytics'):
                                                ...
                                                assert result.get("langfuse_trace_id") == "dict-trace-id-456"
                                    ...
                                    with patch.object(agent, '_process_with_llm', new_callable=AsyncMock) as mock_llm:
                                        with patch.object(agent, '_store_conversation', new_callable=AsyncMock):
                                            with patch.object(agent, '_track_analytics'):
                                                ...
                                                assert result.get("langfuse_trace_id") == "trace-id-attr-789"
                                            ...
                                            with patch.object(agent, '_process_with_llm', new_callable=AsyncMock) as mock_llm:
                                                with patch.object(agent, '_store_conversation', new_callable=AsyncMock):
                                                    with patch.object(agent, '_track_analytics'):
                                                        ...
                                                        assert update_call[1]["trace_id"] == "test-trace-id"
