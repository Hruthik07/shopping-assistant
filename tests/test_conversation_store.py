...
class TestConversationStoreAsync:
    ...
    async def test_get_context_for_llm_is_async(self, store):
        ...
        with patch.object(
            store, "get_conversation_history", new_callable=AsyncMock
        ) as mock_history:
    ...
    async def test_get_context_for_llm_formats_correctly(self, store):
        ...
        with patch.object(
            ...
            mock_history.return_value = [{"user_message": "Hello", "agent_response": "Hi there!"}]
    ...
    async def test_global_conversation_store(self):
        ...
        with patch.object(
            conversation_store, "get_conversation_history", new_callable=AsyncMock
        ) as mock_history:
            mock_history.return_value = [
