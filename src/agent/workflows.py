...
class WorkflowManager:
    ...
    def create_question_workflow(
        self, query: str, product_id: Optional[str] = None
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "steps": [
                {"action": "understand_query", "input": query},
                {
                    "action": "api_retrieve_context",
                    "input": {"query": query, "product_id": product_id},
                },
    ...
    def execute_step(self, workflow: Dict[str, Any], step_data: Any = None) -> Dict[str, Any]:
        ...
        if workflow["current_step"] >= len(workflow["steps"]):
            return {"complete": True, "result": workflow.get("state", {}).get("result")}
