...
Remember: Help users find the perfect products within their budget while maintaining conversation context."""
    ...
    async def process_query(
        ...
        try:
            ...
            history, preferences = await asyncio.gather(
                get_history_with_span(), get_preferences_with_span()
            )
    ...
    async def _process_with_llm(
        ...
        if products:
            ...
            logger.info(
                f"Products extracted from response - Sample keys: {list(sample_product.keys())[:20]}"
            )
    ...
    def _generate_contextual_followup(self, query: str, products: List[Dict[str, Any]]) -> str:
        ...
        for p in products[:3]:  # Check first 3 products
            ...
            if any(word in name or word in desc for word in ["chess", "checkers", "board game"]):
                ...
                for word in ["face wash", "cleanser", "skincare", "beauty", "cosmetic"]
            ):
                categories.append("skincare")
            elif any(
                word in name or word in desc for word in ["massage", "oil", "lotion", "cream"]
            ):
                ...
                word in name or word in desc for word in ["shoe", "clothing", "apparel", "fashion"]
            ):
        ...
        if most_common == "games" or "chess" in query_lower or "board" in query_lower:
            ...
            if (
                "headphone" in query_lower
                or "earbud" in query_lower
                or "earphone" in query_lower
                or "speaker" in query_lower
            ):
                if "gym" in query_lower or "workout" in query_lower or "sport" in query_lower:
        ...
        try:
            ...
            for idx, p in enumerate(products):
                ...
                if price_comp.get("retailer_count", 0) > 1:
                    ...
                    if savings > 0:
                        badges.append(
                            f"⭐ Best Price ({price_comp.get('retailer_count')} retailers, save {savings:.0f}%)"
                        )
                    else:
                ...
                if deal_info.get("is_deal"):
                    ...
                    if savings > 0:
                        if savings_amount > 0:
                            reasons.append(
                                f"Save {savings:.1f}% (${savings_amount:.2f}) vs 30-day average"
                            )
                        else:
                ...
                if len(retailer_options) > 1:
                    ...
                    lines.append(
                        f"   - Available at {len(retailer_options)} retailers (prices: {price_range})"
                    )

                    # Show top 3 retailers with prices
                    sorted_options = sorted(
                        retailer_options, key=lambda x: x.get("total_cost", float("inf"))
                    )[:3]
                    retailer_list = ", ".join(
                        [
