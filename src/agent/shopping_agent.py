...
Remember: Help users find the perfect products within their budget while maintaining conversation context."""
    ...
    def _validate_and_clean_response(self, response_text: str) -> str:
        ...
        if urls:
            ...
            valid_urls = []
            for url in urls:
                is_suspicious = any(re.search(pattern, url, re.IGNORECASE) for pattern in suspicious_patterns)
                if is_suspicious:
                    logger.warning(
            ...
            if 'product' in response_text.lower() and ('buy' in response_text.lower() or 'purchase' in response_text.lower()):
                logger.debug(
    ...
    def _ground_response_to_products(
        ...
        try:
            ...
            lines.append("Want me to narrow it down (budget, hair/skin type, fragrance-free, etc.)?")
            return "\n".join(lines).strip()
        except Exception as e:
