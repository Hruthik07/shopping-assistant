...
class ShoppingAgent:
    ...
    def _create_langchain_tools(self) -> List[StructuredTool]:
        ...
        tools = []
        tools_config = [
            ("search_products", "Search for products by query. Returns list of products with details."),
    ...
    def _get_system_prompt(self, persona: Optional[str] = "friendly", tone: Optional[str] = "warm") -> str:
        ...
        persona_instructions = personas.get((persona or "friendly").lower(), personas.get("friendly", ""))
...
Remember: Help users find the perfect products within their budget while maintaining conversation context."""
        ...
        if isinstance(content, str):
            try:
                ...
                if isinstance(content_dict, dict) and "products" in content_dict:
                    ...
                    if products:
                        logger.info(f"Extracted {len(products)} products from ToolMessage (JSON string)")
                        return products
    ...
    def _should_refetch_products(
        ...
        if matching_count < 3:
            logger.info(f"Only {matching_count} products match price range, refetching with filters")
            return True
    ...
    async def process_query(
        ...
        try:
            ...
            # Use context managers to properly track span duration
            async def get_history_with_span():
                if trace_id:
                    with langfuse_client.span(
                        ...
                        return await self._get_conversation_history(session_id, request_id)
                else:
                    return await self._get_conversation_history(session_id, request_id)
            
            async def get_preferences_with_span():
                if trace_id:
                    with langfuse_client.span(
                        trace_id=trace_id,
                        ...
                        return await self._get_user_preferences(user_id, request_id)
                else:
                    return await self._get_user_preferences(user_id, request_id)
            
            # Fetch history and preferences in parallel with proper span tracking
            history, preferences = await asyncio.gather(
                get_history_with_span(),
                get_preferences_with_span()
            )
            
            # Track intent detection with span
            if trace_id:
                with langfuse_client.span(
                    trace_id=trace_id,
                    name="detect_intent",
            ...
            if history:
                logger.debug(f"Last user message in history: {history[-1].get('user_message', '')[:100]}")
                logger.debug(f"Last agent response in history: {history[-1].get('agent_response', '')[:100]}")
            ...
            if getattr(settings, "semantic_only_retrieval", False):
                ...
                try:
                    semantic_response = await semantic_cache.get_semantic(query, context_hash=context_hash)
                ...
                cached_response = await cache_service.get_llm_response(query, context_hash=context_hash)
            ...
            if cached_response:
                ...
                if getattr(settings, "semantic_only_retrieval", False):
                    ...
                    await self._cache_llm_response(query, context_hash, agent_response, products, tools_used)
            
            # Refetch products if needed
            products_before_refetch = len(products)
            products = await self._ensure_products_with_price_filter(query, products, intent, tools_used)
            ...
            # Store conversation
            await self._store_conversation(session_id, query, agent_response, tools_used, request_id)
                ...
                try:
                    ...
                    if getattr(settings, 'bedrock_enabled', False):
                        metadata.update({
                            ...
                            "cloudwatch_namespace": getattr(settings, 'cloudwatch_namespace', 'ShoppingAssistant/Application')
                        })
                    
                    # Add CloudWatch metric link if enabled
                    if getattr(settings, 'cloudwatch_enabled', False):
                        region = getattr(settings, 'aws_region', 'us-east-1')
                        namespace = getattr(settings, 'cloudwatch_namespace', 'ShoppingAssistant/Application')
    ...
    @db_retry
    async def _get_conversation_history(self, session_id: str, request_id: str) -> List[Dict[str, Any]]:
    ...
    @db_retry
    async def _get_user_preferences(self, user_id: Optional[int], request_id: str) -> Dict[str, Any]:
    ...
    def _create_context_hash(self, messages: List) -> str:
        ...
        try:
            message_contents = [msg.content if hasattr(msg, 'content') else str(msg) for msg in messages[-3:]]
            hash_result = hashlib.sha256(json.dumps(message_contents, sort_keys=True).encode()).hexdigest()[:16]
            return hash_result
        except Exception as e:
            raise
    
    def _extract_from_cache(self, cached_response: Dict[str, Any]) -> tuple[str, List[Dict], List[str]]:
    ...
    async def _process_with_llm(
        ...
        llm_input = {
            "messages": [{"role": msg.__class__.__name__.replace("Message", "").lower(), "content": msg.content if hasattr(msg, "content") else str(msg)} for msg in messages[-10:]]  # Last 10 messages
        }
        ...
        with latency_tracker.track_component("llm_processing", request_id):
            try:
                ...
                raise TimeoutError("LLM request timed out. The query may be too complex or the service is slow. Please try again.")
            except Exception as e:
        ...
        tools_used = self._extract_tools_used(response) if response else intent.get("tools_needed", []).copy()
        products = self._extract_products_from_response(response) if response else []
        
        # Log product structure after extraction for debugging
        if products:
            sample_product = products[0]
            logger.info(f"Products extracted from response - Sample keys: {list(sample_product.keys())[:20]}")
            ...
            try:
                ...
                if getattr(settings, 'bedrock_enabled', False):
                    ...
                    if hasattr(response, "response_metadata") and response.response_metadata:
                        ...
                        if bedrock_metadata:
                            generation_metadata["bedrock_invocation_id"] = bedrock_metadata.get("invocation_id")
                ...
                langfuse_client.generation(
                    ...
                    input=llm_input,
                    output={"response": agent_response[:500], "products_count": len(products), "tools_used": tools_used},
                    usage=usage,
        ...
        if (input_tokens == 0 and output_tokens == 0) and response:
            try:
                if hasattr(response, "response_metadata") and response.response_metadata:
                    ...
                    if token_usage:
                        input_tokens = token_usage.get("input_tokens", 0) or token_usage.get("prompt_tokens", 0) or 0
                        output_tokens = token_usage.get("output_tokens", 0) or token_usage.get("completion_tokens", 0) or 0
    ...
    def _generate_contextual_followup(self, query: str, products: List[Dict[str, Any]]) -> str:
        ...
        Returns:
            Context-appropriate follow-up question or empty string
        """
        if not products:
            return ""
        ...
        # Extract common product categories from products
        categories = []
        for p in products[:3]:  # Check first 3 products
            cat = p.get("category", "").lower() if p.get("category") else ""
            ...
            if any(word in name or word in desc for word in ["chess", "checkers", "board game"]):
                categories.append("games")
            elif any(word in name or word in desc for word in ["face wash", "cleanser", "skincare", "beauty", "cosmetic"]):
                categories.append("skincare")
            elif any(word in name or word in desc for word in ["massage", "oil", "lotion", "cream"]):
                categories.append("wellness")
            elif any(word in name or word in desc for word in ["headphone", "earbud", "earphone", "speaker", "audio", "sound", "laptop", "computer", "phone", "tablet", "electronic", "bluetooth"]):
                categories.append("electronics")
            elif any(word in name or word in desc for word in ["shoe", "clothing", "apparel", "fashion"]):
                categories.append("fashion")
            elif cat:
                categories.append(cat)
        
        # Also check query for category hints
        if "headphone" in query_lower or "earbud" in query_lower or "earphone" in query_lower:
            categories.append("electronics")
        if "gym" in query_lower or "workout" in query_lower or "sport" in query_lower:
            # For gym/workout context with electronics, emphasize durability/features
            pass  # Will be handled by electronics category
        
        # Determine most common category
        if not categories:
            # Generic follow-up if no category detected
            if "budget" not in query_lower and "price" not in query_lower:
        ...
        if most_common == "games" or "chess" in query_lower or "board" in query_lower:
            return "Want me to narrow it down by size, material, style, or price range?"
        elif most_common == "skincare" or "face wash" in query_lower or "beauty" in query_lower:
            return "Want me to narrow it down by skin type, fragrance-free, or other preferences?"
        elif most_common == "wellness" or "massage" in query_lower or "oil" in query_lower:
            return "Want me to narrow it down by type, scent, or size?"
        elif most_common == "electronics":
            # Special handling for audio products
            if "headphone" in query_lower or "earbud" in query_lower or "earphone" in query_lower or "speaker" in query_lower:
                if "gym" in query_lower or "workout" in query_lower or "sport" in query_lower:
                    return "Want me to narrow it down by brand, features (waterproof, noise-canceling), or price range?"
                return "Want me to narrow it down by brand, features, or price range?"
            return "Want me to narrow it down by brand, specs, or price range?"
        elif most_common == "fashion":
            return "Want me to narrow it down by size, color, style, or brand?"
        else:
            # Generic follow-up for other categories
            if "budget" not in query_lower and "price" not in query_lower:
                return "Want me to narrow it down by budget, brand, or other preferences?"
            return "Want me to narrow it down by brand, size, or other preferences?"
    ...
    def _ground_response_to_products(
        ...
        try:
            ...
            lines.append("Why these match: ranked by **semantic similarity (embeddings)** to your request.")
            if max_price is not None:
                lines.append(f"Budget filter applied: ≤ **${max_price:.0f}** (when price was available).")
            lines.append("")
            ...
            for idx, p in enumerate(products):
                ...
                url = _clean_display_text(p.get("product_url") or p.get("link") or p.get("url") or "")
                # Always use neutral link text to avoid parser issues
                link = "View Product"
                if isinstance(url, str) and url.startswith(("http://", "https://")):
                    link = f"[View Product]({url})"

                # Build product header with deal badges
                bullet = f"{shown + 1}. **{name}**"
                
                # Show price with deal information
                deal_info = p.get("deal_info", {})
                coupon_info = p.get("coupon_info", {})
                price_comp = p.get("price_comparison", {})
                
                # Determine best price to show with enhanced formatting
                display_price = price_str
                badges = []
                
                # Check for coupon deals first (highest priority)
                if coupon_info.get("has_coupon"):
                    discounted_price = coupon_info.get("discounted_price", 0)
                    original_price = coupon_info.get("original_price", 0)
                    coupon_savings = coupon_info.get("savings_percent", 0)
                    if discounted_price < original_price and coupon_savings > 0:
                        display_price = f"${discounted_price:.2f}"
                        badges.append(f"💰 Coupon: Save {coupon_savings:.0f}%")
                        if original_price > 0:
                            badges.append(f"(was ${original_price:.2f})")
                
                # Check for price drop deals
                if deal_info.get("is_deal") and not coupon_info.get("has_coupon"):
                    savings = deal_info.get("savings_percent", 0)
                    if savings > 0:
                        badge = deal_info.get("deal_badge", "")
                        if deal_info.get("is_limited_time"):
                            badges.append(f"🏷️ {badge} - Limited Time")
                        elif deal_info.get("is_seasonal"):
                            badges.append(f"🏷️ {badge} - Seasonal Deal")
                        else:
                            badges.append(f"🏷️ {badge}")
                
                # Check for best price across retailers
                if price_comp.get("retailer_count", 0) > 1:
                    savings = price_comp.get("savings_percent", 0)
                    if savings > 0:
                        badges.append(f"⭐ Best Price ({price_comp.get('retailer_count')} retailers, save {savings:.0f}%)")
                    else:
                        badges.append(f"⭐ Compare {price_comp.get('retailer_count')} retailers")
                
                # Build final price display
                if badges:
                    bullet += f" - {display_price} {' | '.join(badges)}"
                else:
                    if price_str:
                        bullet += f" - {price_str}"
                ...
                elif isinstance(category, str) and category.strip() and category.strip().lower() != "general":
                ...
                # Why it matches (deal info, price comparison, semantic score)
                reasons: List[str] = []
                
                # Add deal information with enhanced details
                if deal_info.get("is_deal"):
                    savings = deal_info.get("savings_percent", 0)
                    savings_amount = deal_info.get("savings_amount", 0)
                    if savings > 0:
                        if savings_amount > 0:
                            reasons.append(f"Save {savings:.1f}% (${savings_amount:.2f}) vs 30-day average")
                        else:
                            reasons.append(f"Save {savings:.1f}% vs average price")
                    if deal_info.get("is_lowest_price"):
                        reasons.append("Lowest price in 30 days")
                    if deal_info.get("is_limited_time"):
                        reasons.append("Limited time offer")
                    if deal_info.get("is_seasonal"):
                        reasons.append("Seasonal sale")
                
                # Add coupon information
                if coupon_info.get("has_coupon"):
                    coupon_savings = coupon_info.get("savings_percent", 0)
                    if coupon_savings > 0:
                        reasons.append(f"Coupon saves {coupon_savings:.1f}%")
                
                # Add price comparison
                if price_comp.get("retailer_count", 0) > 1:
                    savings = price_comp.get("savings_percent", 0)
                    if savings > 0:
                        reasons.append(f"Best price among {price_comp.get('retailer_count')} retailers")
                ...
                try:
                    ...
                    score_val = None
                if score_val is not None and not reasons:
                ...
                # Show multiple retailer options if available
                retailer_options = p.get("retailer_options", [])
                if len(retailer_options) > 1:
                    prices = [opt.get("total_cost", 0) for opt in retailer_options]
                    min_price = min(prices)
                    max_price = max(prices)
                    price_range = f"${min_price:.2f} to ${max_price:.2f}"
                    if min_price == max_price:
                        price_range = f"${min_price:.2f}"
                    lines.append(f"   - Available at {len(retailer_options)} retailers (prices: {price_range})")
                    
                    # Show top 3 retailers with prices
                    sorted_options = sorted(retailer_options, key=lambda x: x.get("total_cost", float('inf')))[:3]
                    retailer_list = ", ".join([
                        f"{opt.get('retailer', 'Unknown')} (${opt.get('total_cost', 0):.2f})"
                        for opt in sorted_options
                    ])
                    if retailer_list:
                        lines.append(f"   - Top retailers: {retailer_list}")
            ...
            # Generate context-aware follow-up question based on product categories
            follow_up = self._generate_contextual_followup(query, products)
            if follow_up:
    ...
    def _validate_products_against_tools(
        ...
        for product in products:
            ...
            if not product.get("name") and not product.get("title"):
                logger.warning(f"Skipping product without name/title - potential hallucination: {product}")
                continue
            
            # Check if product has a source (should come from tools)
            if not product.get("source") and not product.get("id"):
                logger.debug(f"Product without source field - may be hallucinated: {product.get('name', 'Unknown')}")
    ...
    async def _ensure_products_with_price_filter(
        ...
        if "search_products" not in intent.get("tools_needed", []) and "search_products" not in tools_used:
            return products
        ...
        fallback_products = await self._refetch_products_with_price_filter(query, min_price, max_price)
        
        if not fallback_products:
            return products
        
        logger.info(f"Fallback search returned {len(fallback_products)} products (with price filtering)")
    ...
    def _filter_products_by_brand(
        ...
        for pattern in exclusion_patterns:
            ...
            for match in matches:
                ...
                if len(brand) > 2 and (brand[0].isupper() or brand.lower() in ['nike', 'adidas', 'puma', 'reebok', 'bose', 'sony', 'apple', 'samsung']):
        ...
        for pattern in inclusion_patterns:
            ...
            for match in matches:
                brand = match.strip()
                if len(brand) > 2 and (brand[0].isupper() or brand.lower() in ['nike', 'adidas', 'puma', 'reebok', 'bose', 'sony', 'apple', 'samsung', 'jbl']):
        ...
        for product in products:
            ...
            for excluded in excluded_brands:
                if excluded in product_name or excluded in product_brand:
                    should_exclude = True
                    logger.debug(f"Excluding product '{product.get('name', '')}' - matches excluded brand: {excluded}")
                    break
            ...
            if included_brands:
                ...
                for included in included_brands:
                    if included in product_name or included in product_brand:
                        should_include = True
                        logger.debug(f"Including product '{product.get('name', '')}' - matches included brand: {included}")
                        break
        ...
        if excluded_brands:
            logger.info(f"Brand exclusion filter: {len(products)} -> {len(filtered_products)} products (excluded: {excluded_brands})")
        if included_brands:
            logger.info(f"Brand inclusion filter: {len(products)} -> {len(filtered_products)} products (included: {included_brands})")
        
        return filtered_products if filtered_products else products  # Return original if all filtered out
