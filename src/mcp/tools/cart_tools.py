...
class AddToCartTool(MCPTool):
    ...
    async def execute(self, product_id: str, quantity: int = 1, user_id: Optional[int] = None, db: Optional[Session] = None) -> Dict[str, Any]:
        """Add product to cart."""
        should_close = False
        if db is None:
            db = SessionLocal()
...
class RemoveFromCartTool(MCPTool):
    ...
    async def execute(self, user_id: int, cart_item_id: int, db: Optional[Session] = None) -> Dict[str, Any]:
        """Remove item from cart."""
        should_close = False
        if db is None:
            db = SessionLocal()
