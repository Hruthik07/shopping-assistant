...
class ImageAnalysisTool(MCPTool):
    ...
    def get_parameters(self) -> Dict[str, Any]:
        return {
            "type": "object",
            "properties": {
                "image_url": {"type": "string", "description": "URL of the product image"},
                "image_base64": {
                    "type": "string",
    ...
    async def execute(
        self, image_url: Optional[str] = None, image_base64: Optional[str] = None
    ) -> Dict[str, Any]:
    ...
    async def execute(self, description: str, max_results: int = 5) -> Dict[str, Any]:
        ...
        try:
            products = await product_fetcher.search_products(
                query=description, num_results=max_results, use_google_shopping=True
            )
            ...
            return {"description": description, "results_count": 0, "products": [], "error": str(e)}
