...
class HybridRetriever:
    ...
    def retrieve_products(
        self, query: str, n_results: int = 5, category_filter: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        ...
        with latency_tracker.track_component("reranking"):
            ...
            for result in semantic_results:
                ...
                keyword_score = calculate_similarity_score(query, result["document"])
                semantic_score = 1 - (result["distance"] or 0)  # Convert distance to similarity

                # Weighted combination (70% semantic, 30% keyword)
                combined_score = 0.7 * semantic_score + 0.3 * keyword_score

                scored_results.append({**result, "score": combined_score})
    ...
    def retrieve_reviews(
        self, query: str, product_id: Optional[str] = None, n_results: int = 3
    ) -> List[Dict[str, Any]]:
        """Retrieve reviews using semantic search."""
        query_embedding = self.embedding_generator.embed_text(query)

        results = self.vector_store.search_reviews(
            query_embedding, product_id=product_id, n_results=n_results
        )
    ...
    async def get_product_context(
        self, product_id: str, include_reviews: bool = True, include_faqs: bool = True
    ) -> Dict[str, Any]:
        ...
        context = {"product": product, "reviews": [], "faqs": []}

        if include_reviews and product:
            reviews = self.retrieve_reviews(
                query=product.get("name", ""), product_id=product_id, n_results=3
            )
