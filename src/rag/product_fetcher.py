...
class ProductFetcher:
    ...
    async def _fetch_from_serper_api(self, query: str, num_results: int) -> List[Dict[str, Any]]:
    ...
    async def _fallback_to_cache(self, query: str, num_results: int) -> List[Dict[str, Any]]:
    ...
    @async_circuit_breaker(
        config=CircuitBreakerConfig(failure_threshold=5, recovery_timeout=30, name="serper_api"),
        fallback=None,  # Will use method-level fallback
    )
    @async_retry(
        config=RetryConfig(max_attempts=3, initial_wait=1.0, max_wait=10.0),
        fallback=None,  # Will use method-level fallback
    )
    ...
    async def get_product_details(
        self, product_url: str, source: str = "google_shopping"
    ...
    async def search_products(
        ...
        if use_tavily and len(all_products) < num_results:
            tavily_products = await self.search_products_tavily(
                query, num_results - len(all_products)
            )
