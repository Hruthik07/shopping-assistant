...
class ShoppingAssistantGuardrails:
    ...
    def __init__(self):
        self.blocked_patterns = [re.compile(pattern, re.IGNORECASE) for pattern in self.BLOCKED_PATTERNS]
    ...
    def validate_query(self, query: str) -> Tuple[bool, Optional[str]]:
        ...
        _debug_log('guardrails.py:48', 'validate_query called', {'query_length': len(query) if query else 0, 'query_type': type(query).__name__, 'query_preview': query[:50] if query else None}, 'K')
        # #endregion
        if not query or not isinstance(query, str):
            # #region debug instrumentation
            _debug_log('guardrails.py:56', 'validate_query failed - not string or empty', {'query': query}, 'K')
            # #endregion
        ...
        if len(query) > self.MAX_QUERY_LENGTH:
            # #region debug instrumentation
            _debug_log('guardrails.py:61', 'validate_query failed - too long', {'query_length': len(query), 'max_length': self.MAX_QUERY_LENGTH}, 'K')
            # #endregion
        ...
        for pattern in self.blocked_patterns:
            if pattern.search(query):
                ...
                # #region debug instrumentation
                _debug_log('guardrails.py:66', 'validate_query blocked - pattern match', {'pattern': pattern.pattern, 'query_preview': query[:50]}, 'K')
                # #endregion
                return False, "I can't help with that type of request. Please ask about legitimate shopping needs."
        ...
        for pattern in self.suspicious_patterns:
            if pattern.search(query):
                ...
                # #region debug instrumentation
                _debug_log('guardrails.py:72', 'validate_query suspicious pattern', {'pattern': pattern.pattern}, 'K')
                # #endregion
    ...
    def validate_price_range(self, min_price: Optional[float], max_price: Optional[float]) -> Tuple[bool, Optional[str]]:
        """Validate price range parameters."""
        # #region debug instrumentation
        _debug_log('guardrails.py:75', 'validate_price_range called', {'min_price': min_price, 'max_price': max_price, 'max_allowed': self.MAX_PRICE}, 'K')
        # #endregion
        if min_price is not None:
            if min_price < 0:
                # #region debug instrumentation
                _debug_log('guardrails.py:79', 'validate_price_range failed - negative min', {'min_price': min_price}, 'K')
                # #endregion
                return False, "Minimum price cannot be negative"
            if min_price > self.MAX_PRICE:
                # #region debug instrumentation
                _debug_log('guardrails.py:83', 'validate_price_range failed - min too high', {'min_price': min_price, 'max_allowed': self.MAX_PRICE}, 'K')
                # #endregion
        ...
        if max_price is not None:
            if max_price < 0:
                # #region debug instrumentation
                _debug_log('guardrails.py:88', 'validate_price_range failed - negative max', {'max_price': max_price}, 'K')
                # #endregion
                return False, "Maximum price cannot be negative"
            if max_price > self.MAX_PRICE:
                # #region debug instrumentation
                _debug_log('guardrails.py:92', 'validate_price_range failed - max too high', {'max_price': max_price, 'max_allowed': self.MAX_PRICE}, 'K')
                # #endregion
                return False, f"Maximum price limit is ${self.MAX_PRICE:,.2f}"
        ...
        if min_price is not None and max_price is not None:
            if min_price > max_price:
                # #region debug instrumentation
                _debug_log('guardrails.py:98', 'validate_price_range failed - min > max', {'min_price': min_price, 'max_price': max_price}, 'K')
                # #endregion
                return False, "Minimum price cannot be greater than maximum price"
        
        # #region debug instrumentation
        _debug_log('guardrails.py:102', 'validate_price_range passed', {'min_price': min_price, 'max_price': max_price}, 'K')
        # #endregion
        return True, None
    
    def validate_tool_usage(self, tool_name: str, tool_calls_count: int) -> Tuple[bool, Optional[str]]:
        """Validate tool usage limits."""
        if tool_calls_count >= self.MAX_TOOL_CALLS_PER_REQUEST:
            return False, f"Too many tool calls. Maximum {self.MAX_TOOL_CALLS_PER_REQUEST} calls per request."
    ...
    def validate_product_data(self, products: List[Dict[str, Any]]) -> Tuple[bool, Optional[str]]:
        ...
        if len(products) > self.MAX_PRODUCTS_PER_RESPONSE:
            logger.warning(f"Too many products returned: {len(products)}. Limiting to {self.MAX_PRODUCTS_PER_RESPONSE}")
            return False, f"Too many products. Maximum {self.MAX_PRODUCTS_PER_RESPONSE} products per response."
    ...
    def _is_safe_url(self, url: str) -> bool:
        ...
        # #region debug instrumentation
        _debug_log('guardrails.py:138', '_is_safe_url called', {'url': url[:100] if url else None, 'url_type': type(url).__name__}, 'K')
        # #endregion
        if not isinstance(url, str):
            # #region debug instrumentation
            _debug_log('guardrails.py:141', '_is_safe_url failed - not string', {'url_type': type(url).__name__}, 'K')
            # #endregion
            return False
        ...
        if not url.startswith(("http://", "https://")):
            # #region debug instrumentation
            _debug_log('guardrails.py:146', '_is_safe_url failed - invalid scheme', {'url': url[:50]}, 'K')
            # #endregion
            return False
        ...
        if url.startswith(("javascript:", "data:", "file:")):
            # #region debug instrumentation
            _debug_log('guardrails.py:151', '_is_safe_url failed - blocked scheme', {'url': url[:50]}, 'K')
            # #endregion
            return False
    ...
    def sanitize_response_with_products(self, response: str, products: List[Dict[str, Any]]) -> str:
        ...
        mentioned = [m for m in merchant_domains.keys() if re.search(rf"\b{re.escape(m)}\b", text_lower)]
        ...
        logger.warning(f"Merchant hallucination detected in response: {bad}. Neutralizing store mentions.")

        # Neutralize store mentions with broader patterns (covers "View at Walmart", "Check on Target", etc.)
        for m in bad:
            response = re.sub(rf"(?i)\b(buy|purchase|order)\s+(on|from)\s+{re.escape(m)}\b", "Buy here", response)
            response = re.sub(rf"(?i)\b(view|check|shop)\s+(on|at|from)\s+{re.escape(m)}\b", "View Product", response)
            response = re.sub(rf"(?i)\bavailable\s+(on|at)\s+{re.escape(m)}\b", "Available online", response)
            response = re.sub(rf"(?i)\b(where to buy|buy at|shop at)\s*:\s*{re.escape(m)}\b", "Buy here", response)
