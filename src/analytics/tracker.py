...
class AgentTracker:
    ...
    def track_deal_detection(
        ...
        if deals_found > 0:
            ...
            if current_count > 0:
                total_avg = (current_avg * current_count + average_savings_percent * deals_found) / self.metrics["deals_detected"]
            else:
    ...
    def track_price_comparison(
        ...
        if self.metrics["price_comparisons"] > 0:
            ...
            if current_count > 0:
                total_avg = (current_avg * current_count + average_retailers * products_compared) / self.metrics["price_comparisons"]
            else:
        ...
        if products_compared > 0:
            ...
            current_count = self.metrics["price_comparisons"] - products_compared
            if current_count > 0:
                total_avg = (current_avg * current_count + average_price_diff * products_compared) / self.metrics["price_comparisons"]
            else:
                total_avg = average_price_diff
            self.metrics["average_price_difference"] = total_avg
        
        logger.debug(f"Price comparison tracked: {products_compared} products, {products_with_multiple_retailers} with multiple retailers")
