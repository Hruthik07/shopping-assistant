...
PRICING = {
    ...
    "default": {"input": 0.25, "output": 1.25, "provider": "anthropic"},
}
...
class CostTracker:
    ...
    def get_cost_stats(self, days: int = 7, model: Optional[str] = None) -> Dict[str, Any]:
        ...
        # Filter history
        filtered_history = [entry for entry in self.cost_history if entry["date"] >= cutoff_date]

        if model:
            filtered_history = [entry for entry in filtered_history if entry["model"] == model]
        ...
        return {
            ...
            "average_tokens_per_request": (
                round(total_tokens / request_count, 2) if request_count > 0 else 0
            ),
    ...
    def get_recent_costs(self, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        """
        return sorted(self.cost_history, key=lambda x: x["timestamp"], reverse=True)[:limit]

    def check_budget_alert(
        self, daily_budget: Optional[float] = None, weekly_budget: Optional[float] = None
