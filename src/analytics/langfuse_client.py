        ...
        if self.enabled:
            try:
                if settings.langfuse_public_key and settings.langfuse_secret_key:
                    ...
                    logger.info(f"Langfuse client initialized for project: {settings.langfuse_project_name}")
                else:
    ...
    def score(
        ...
        try:
            ...
            # Fallback: try score_current_trace if we're in trace context
                ...
                logger.warning(f"Using score_current_trace fallback for {name} - trace_id may not be used")
                ...
                logger.warning(f"Langfuse client has no score method (create_score or score_current_trace)")
                return None
        except Exception as e:
