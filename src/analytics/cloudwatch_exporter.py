...
class CloudWatchExporter:
    ...
    def _initialize_client(self):
        ...
        try:
            ...
            logger.warning("boto3 not installed. CloudWatch export disabled. Install with: pip install boto3")
            self.enabled = False
        except Exception as e:
            logger.warning(f"Failed to initialize CloudWatch client: {e}. CloudWatch export disabled.")
            self.enabled = False
    ...
    async def put_metric(
        ...
        if (len(self.metric_buffer) >= 20 or 
