...
class PerformanceMonitor:
    ...
    def check_regression(
        ...
        if self.baseline_p95 and percentiles["p95"] > self.baseline_p95 * 1.2:
            alerts.append({
                ...
                "increase_percent": ((percentiles["p95"] - self.baseline_p95) / self.baseline_p95) * 100,
                "severity": "high" if percentiles["p95"] > self.baseline_p95 * 1.5 else "medium"
            })
        ...
        if self.baseline_p99 and percentiles["p99"] > self.baseline_p99 * 1.2:
            alerts.append({
                ...
                "increase_percent": ((percentiles["p99"] - self.baseline_p99) / self.baseline_p99) * 100,
                "severity": "high" if percentiles["p99"] > self.baseline_p99 * 1.5 else "medium"
            })
    ...
    def get_slow_queries(self, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        return [
            {"latency": lat, "rank": i + 1}
            for i, lat in enumerate(sorted_latencies[:limit])
        ]
