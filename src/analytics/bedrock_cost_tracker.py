...
class BedrockCostTracker:
    ...
    def _initialize_client(self):
        ...
        try:
            ...
            logger.warning("boto3 not installed. Bedrock cost tracking disabled. Install with: pip install boto3")
            self.enabled = False
        except Exception as e:
            logger.warning(f"Failed to initialize Cost Explorer client: {e}. Bedrock cost tracking disabled.")
            self.enabled = False
    ...
    async def get_bedrock_costs(
        ...
        try:
            ...
            return {
                ...
                "currency": response.get('ResultsByTime', [{}])[0].get('Total', {}).get('UnblendedCost', {}).get('Unit', 'USD')
            }
            ...
            return {
                "enabled": False,
                "error": f"Cost Explorer API error: {error_code}"
            }
        except Exception as e:
