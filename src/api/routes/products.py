...
async def search_products(
    ...
    limit: Optional[int] = Query(
        None, ge=1, le=50, description="Maximum results (alias: max_results)"
    ),
    max_results: Optional[int] = Query(
        None, ge=1, le=50, description="Maximum results (alias for limit)"
    ),
):
    """Search for products."""
    try:
        ...
        if not q_effective:
            raise HTTPException(
                status_code=422, detail="Missing required query parameter: q (or query)"
            )
        ...
        api_products = await product_fetcher.search_products(
            query=q_effective, num_results=limit_effective, use_google_shopping=True
        )

        # Filter by category if specified
        if category:
            api_products = [
                p for p in api_products if p.get("category", "").lower() == category.lower()
            ]
