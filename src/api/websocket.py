                    ...
                    try:
                        ...
                        async for chunk in agent.stream_response(messages):
                            ...
                            await manager.send_personal_message(
                                json.dumps({"type": "chunk", "content": chunk}), websocket
                            )

                        # Send completion signal
                        await manager.send_personal_message(
                            json.dumps({"type": "complete", "response": full_response}), websocket
                        )
                        ...
                        result = await agent.process_query(query=data, session_id=session_id)

                        # Guardrail: Sanitize response (with product context for faithfulness)
                        response = guardrails.sanitize_response_with_products(
                            result.get("response", ""), result.get("products", []) or []
                        )

                        # Send response
                        await manager.send_personal_message(
                            json.dumps({"type": "complete", "response": response}), websocket
                        )

                except Exception as agent_error:
                    logger.error(
                        f"Error processing query in WebSocket: {agent_error}", exc_info=True
                    )
