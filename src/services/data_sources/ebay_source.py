...
class eBayDataSource(ProductDataSource):
    ...
    def normalize_product(self, product: Dict[str, Any]) -> Dict[str, Any]:
        ...
        item_id = (
            ...
            if isinstance(product.get("itemId"), list)
            else product.get("itemId", "")
        )
        title = (
            product.get("title", [""])[0]
            if isinstance(product.get("title"), list)
            else product.get("title", "")
        )

        # Extract price
        selling_status = (
            product.get("sellingStatus", [{}])[0]
            if isinstance(product.get("sellingStatus"), list)
            else product.get("sellingStatus", {})
        )
        current_price = (
            selling_status.get("currentPrice", [{}])[0]
            if isinstance(selling_status.get("currentPrice"), list)
            else selling_status.get("currentPrice", {})
        )
        price_value = (
            float(current_price.get("__value__", 0)) if isinstance(current_price, dict) else 0.0
        )
        currency = (
            current_price.get("@currencyId", "USD") if isinstance(current_price, dict) else "USD"
        )

        # Extract shipping
        shipping_info = (
            product.get("shippingInfo", [{}])[0]
            if isinstance(product.get("shippingInfo"), list)
            else product.get("shippingInfo", {})
        )
        shipping_cost = 0.0
        if shipping_info.get("shippingServiceCost", [{}])[0].get("__value__"):
            shipping_cost = float(
                shipping_info.get("shippingServiceCost", [{}])[0].get("__value__", 0)
            )

        # Extract image
        gallery_url = (
            product.get("galleryURL", [""])[0]
            if isinstance(product.get("galleryURL"), list)
            else product.get("galleryURL", "")
        )
        view_item_url = (
            product.get("viewItemURL", [""])[0]
            if isinstance(product.get("viewItemURL"), list)
            else product.get("viewItemURL", "")
        )
        ...
        return {
            ...
            "reviews": None,
            "category": (
                product.get("primaryCategoryName", [""])[0]
                if isinstance(product.get("primaryCategoryName"), list)
                else "general"
            ),
            "brand": None,
            "availability": True,
