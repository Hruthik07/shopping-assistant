...
class PriceTracker:
    ...
    async def record_price(self, product: Dict[str, Any], db: Optional[Session] = None) -> None:
    ...
    async def get_price_history(
        ...
        try:
            ...
            query = db.query(PriceHistory).filter(
                PriceHistory.product_id == product_id, PriceHistory.timestamp >= cutoff_date
            )
    ...
    async def cleanup_old_records(
        self, days: Optional[int] = None, db: Optional[Session] = None
    ) -> int:
        ...
        try:
            ...
            # Delete old records
            deleted = db.query(PriceHistory).filter(PriceHistory.timestamp < cutoff_date).delete()

            db.commit()
            logger.info(
                f"Cleaned up {deleted} old price history records (older than {retention_days} days)"
            )
            return deleted
