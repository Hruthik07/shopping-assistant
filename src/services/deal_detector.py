...
class DealDetector:
    ...
    async def detect_deals(
        ...
        for product in products:
            try:
                ...
                logger.error(f"Error detecting deals for product {product.get('id', 'unknown')}: {e}", exc_info=True)
        ...
        if products_with_deals:
            ...
            deals_found = sum(1 for p in products_with_deals if p.get("deal_info", {}).get("is_deal", False))
            total_savings = sum(p.get("deal_info", {}).get("savings_amount", 0) for p in products_with_deals)
            avg_savings = sum(p.get("deal_info", {}).get("savings_percent", 0) for p in products_with_deals if p.get("deal_info", {}).get("is_deal", False))
    ...
    async def _analyze_product_deals(self, product: Dict[str, Any]) -> Dict[str, Any]:
        ...
        product["deal_info"] = {
            ...
            "savings_amount": round(average_price - total_cost, 2) if average_price > total_cost else 0.0,
