...
class PromoMatcher:
            ...
            try:
                ...
                try:
                    coupons = await coupon_service.find_coupons(
                        retailer=retailer, product_url=product.get("product_url")
                    )
                ...
                if coupons:
                    ...
                    try:
                        ...
                        discount_info = coupon_service.calculate_discounted_price(
                            product.get("price", 0.0), best_coupon
                        )
                        ...
                        product["coupon_info"] = {
                            ...
                            "discounted_price": discount_info.get(
                                "discounted_price", product.get("price", 0.0)
                            ),
                        ...
                        product["price_after_coupon"] = discount_info.get(
                            "discounted_price", product.get("price", 0.0)
                        )
                    except Exception as e:
                ...
                logger.error(
                    f"Error matching promos for product {product.get('id', 'unknown')}: {e}",
                    exc_info=True,
                )
    ...
    def _select_best_coupon(
        self, coupons: List[Dict[str, Any]], original_price: float
    ) -> Dict[str, Any]:
