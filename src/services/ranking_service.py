...
class RankingService:
    ...
    def rank_products(
        ...
        for product in products:
            try:
                ...
                logger.error(f"Error calculating customer value for product {product.get('id', 'unknown')}: {e}", exc_info=True)
        ...
        if sort_by == "customer_value":
            sorted_products = sorted(
                products,
                key=lambda p: p.get("customer_value", {}).get("score", 0),
                reverse=True
            )
        elif sort_by == "price":
        ...
        for idx, product in enumerate(sorted_products, 1):
            product["rank"] = idx
            product["ranking_explanation"] = self._generate_ranking_explanation(
                product,
                sort_by
            )
    ...
    def _generate_ranking_explanation(
        ...
        if price_comp.get("retailer_count", 0) > 1:
            ...
            if savings > 0:
                explanations.append(f"Best price among {price_comp.get('retailer_count')} retailers")
