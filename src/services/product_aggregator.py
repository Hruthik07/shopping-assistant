...
class ProductAggregator:
    ...
    def __init__(self):
        ...
        priority_order = settings.product_source_priority.split(",") if hasattr(settings, "product_source_priority") and settings.product_source_priority else ["serper"]
        source_priority_map = {
        ...
        logger.info(f"Initialized ProductAggregator with {len(self.sources)} data sources: {[s.get_source_name() for s in self.sources]}")
        ...
        try:
            ...
            for idx, result in enumerate(results):
                ...
                if isinstance(result, list):
                    ...
                    logger.warning(f"Source {self.sources[idx].get_source_name()} returned unexpected type: {type(result)}")
            
            if successful_sources == 0:
                logger.error("All data sources failed, returning empty results")
                return []
            
            logger.info(f"Successfully queried {successful_sources}/{len(self.sources)} data sources")
            ...
            # Apply filters
            try:
                filtered_products = self._apply_filters(merged_products, min_price, max_price, category)
            except Exception as e:
