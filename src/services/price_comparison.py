...
class PriceComparator:
    ...
    def compare_prices(self, products: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for product in products:
            try:
                ...
                logger.error(
                    f"Error comparing prices for product {product.get('id', 'unknown')}: {e}",
                    exc_info=True,
                )
        ...
        if compared_products:
            ...
            products_with_multiple = sum(
                1
                for p in compared_products
                if p.get("price_comparison", {}).get("retailer_count", 0) > 1
            )
            avg_retailers = sum(
                p.get("price_comparison", {}).get("retailer_count", 1) for p in compared_products
            ) / len(compared_products)
            avg_diff = sum(
                p.get("price_comparison", {}).get("savings", 0) for p in compared_products
            ) / len(compared_products)
    ...
    def _compare_retailer_options(self, product: Dict[str, Any]) -> Dict[str, Any]:
        ...
        sorted_options = sorted(retailer_options, key=lambda x: x.get("total_cost", float("inf")))
    ...
    def _add_comparison_metadata(self, product: Dict[str, Any]) -> Dict[str, Any]:
        ...
        product["price_comparison"] = {
            ...
            "retailer_count": 1,
            "price_range": {"min": total_cost, "max": total_cost},
        }

        return product

    def calculate_total_cost(
        self, price: float, shipping_cost: float = 0.0, tax: float = 0.0, discount: float = 0.0
    ) -> float:
    ...
    def rank_by_customer_value(self, products: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
